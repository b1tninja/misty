...
if __name__ == '__main__':
    ...
    for filing_code_id, filing_code_desc in tqdm.tqdm(filing_codes.items(), position=0, leave=True):
        ...
        while n == 0 or n < j['ResultCount']:
            ...
            if j is None:
                if retries:
                    logging.warning("Failed to get chunk! (%d retries left)", retries)
                    retries -= 1
                    continue
                else:
                    break
            ...
            logging.info("%d/%d", n, j['ResultCount'])
